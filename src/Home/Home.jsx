...
const MyComponent = () => {
  ...
  const text1 = 'Stree Shakti'
  const text2 = 'A women entrepreneurship platform'
  return (
    <div>
          ...
          <h2>
            {Array.from(text2).map((char, i) => (
              <span
                className="trans"
                key={i + text1.length}
                style={{ animationDelay: `${(i + text1.length) * 0.2}s` }}
              >
                {char}
              </span>
            ))}
